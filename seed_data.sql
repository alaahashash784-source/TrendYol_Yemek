...
-- Get Restaurant IDs
DECLARE @KebabId INT = (SELECT RestoranId FROM Restorans WHERE Ad = 'Turk Kebap Evi');
DECLARE @PizzaId INT = (SELECT RestoranId FROM Restorans WHERE Ad = 'Pizza Cenneti');
DECLARE @BurgerId INT = (SELECT RestoranId FROM Restorans WHERE Ad = 'Burger Bolgesi');
DECLARE @SeafoodId INT = (SELECT RestoranId FROM Restorans WHERE Ad = 'Deniz Lezzetleri');
DECLARE @AsianId INT = (SELECT RestoranId FROM Restorans WHERE Ad = 'Asya Tatlari');
DECLARE @CafeId INT = (SELECT RestoranId FROM Restorans WHERE Ad = 'Tatli Ruyalar Kafe');
