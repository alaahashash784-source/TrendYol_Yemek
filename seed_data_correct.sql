...
-- Get Restaurant IDs
DECLARE @KebabId INT = (SELECT RestoranId FROM Restoranlar WHERE Ad = 'Turkish Kebab House');
DECLARE @PizzaId INT = (SELECT RestoranId FROM Restoranlar WHERE Ad = 'Pizza Paradise');
DECLARE @BurgerId INT = (SELECT RestoranId FROM Restoranlar WHERE Ad = 'Burger Zone');
DECLARE @SeafoodId INT = (SELECT RestoranId FROM Restoranlar WHERE Ad = 'Seafood Delight');
DECLARE @AsianId INT = (SELECT RestoranId FROM Restoranlar WHERE Ad = 'Asian Flavors');
DECLARE @CafeId INT = (SELECT RestoranId FROM Restoranlar WHERE Ad = 'Sweet Dreams Cafe');
...
('Admin', 'User', 'admin@example.com', 'admin123', '+90 532 000 0000', 'Istanbul, Turkey', GETDATE(), 1),
('John', 'Doe', 'john@example.com', 'password123', '+90 533 123 4567', 'Istanbul, Turkey', GETDATE(), 0),
('Jane', 'Smith', 'jane@example.com', 'password123', '+90 534 987 6543', 'Ankara, Turkey', GETDATE(), 0);
